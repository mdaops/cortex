...
kubectl --context kind-cortex create namespace fleet-dev 2>/dev/null || true

kubectl --context kind-cortex -n fleet-dev create secret generic cluster-kubeconfig \
    --from-literal=value="$AXON_KUBECONFIG" \
    --dry-run=client -o yaml | kubectl --context kind-cortex apply -f -

echo "Kubeconfig secret created in fleet-dev namespace"
